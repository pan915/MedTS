...
from input_features import encode_input, encode_utter
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, pretrained_model, device, max_sequence_length, schema_embedding_size, decoder_hidden_size, args):
        ...
        self.encoder_lstm = nn.LSTM(self.encoder_hidden_size, args.hidden_size // 2, bidirectional=True,
                                    batch_first=True)
        ...
        # self.utter_att_linear = nn.Linear(args.hidden_size*2, args.hidden_size, bias=False)

        # self.column_encoder = nn.Linear(transformer_config.hidden_size, decoder_hidden_size)
        # self.table_encoder = nn.Linear(transformer_config.hidden_size, decoder_hidden_size)
        # self.value_encoder = nn.Linear(transformer_config.hidden_size, decoder_hidden_size)
        # self.pooled_encoder = nn.Linear(transformer_config.hidden_size, decoder_hidden_size)
        if args.use_schema_lstm:
            self.column_encoder = nn.LSTM(transformer_config.hidden_size, schema_embedding_size // 2,
                                          bidirectional=True, batch_first=True)
            self.table_encoder = nn.LSTM(transformer_config.hidden_size, schema_embedding_size // 2, bidirectional=True,
                                         batch_first=True)
            self.value_encoder = nn.LSTM(transformer_config.hidden_size, schema_embedding_size // 2, bidirectional=True,
                                         batch_first=True)
        else:
            self.column_encoder = nn.Linear(transformer_config.hidden_size, schema_embedding_size)
            self.table_encoder = nn.Linear(transformer_config.hidden_size, schema_embedding_size)
            self.value_encoder = nn.Linear(transformer_config.hidden_size, schema_embedding_size)

        print("Successfully loaded pre-trained transformer '{}'".format(pretrained_model))

    def encode_utterance(self, question_tokens):
        input_ids_tensor, attention_mask_tensor, segment_ids_tensor, all_question_span_lengths = encode_utter(
            question_tokens,
            self.tokenizer,
            self.max_sequence_length,
            self.device)

        last_hidden_states, pooling_output = self.transformer_model(input_ids_tensor, attention_mask_tensor,
                                                                    segment_ids_tensor)
        averaged_hidden_states_question, pointers_after_question = self._average_hidden_states_question(
            last_hidden_states, all_question_span_lengths)
        question_out = pad_sequence(averaged_hidden_states_question, batch_first=True)
        ...
        if not self.args.use_lstm_encoder:
            ...
            question_out = self.lstm_encode(question_out, question_out.size()[1])

            question_out = (self.dropout(question_out[0]), (question_out[1][0], question_out[1][1]))
        return question_out, pooling_output
    ...
    def forward(self, question_tokens, column_names, table_names, values):
        ...
        # while the "last_hidden-states" is one hidden state per input token, the pooler_output is the
        # hidden state of the [CLS]-token, further processed.
        # See e.g. "BertModel" documentation for more information.

        last_hidden_states, pooling_output = self.transformer_model(input_ids_tensor, attention_mask_tensor,
                                                                    segment_ids_tensor)

        (all_question_span_lengths, all_column_token_lengths, all_table_token_lengths,
         all_value_token_lengths) = input_lengths

        # we get the relevant hidden states for the question-tokens and average, if there are multiple token per  word
        # (e.g ['table', 'college'])
        averaged_hidden_states_question, pointers_after_question = self._average_hidden_states_question(
            last_hidden_states, all_question_span_lengths)
        question_out = pad_sequence(averaged_hidden_states_question, batch_first=True)  # (batch_size * max_question_
        # tokens_per_batch * hidden_size)
        # as the transformer uses normally a size of 768 and the decoder only 300 per vector, we need to reduce
        # dimensionality here with a linear layer.

        column_hidden_states, pointers_after_columns = self._get_schema_hidden_states(last_hidden_states,
                                                                                      all_column_token_lengths,
                                                                                      pointers_after_question)
        table_hidden_states, pointers_after_tables = self._get_schema_hidden_states(last_hidden_states,
                                                                                    all_table_token_lengths,
                                                                                    pointers_after_columns)

        # in this scenario, we know the values upfront and encode them similar to tables/columns. This is
        # different as soon as we don't know the values but have to extract them from the question.
        value_hidden_states, pointers_after_values = self._get_schema_hidden_states(last_hidden_states,
                                                                                    all_value_token_lengths,
                                                                                    pointers_after_tables)

        # This is simply to make sure the rather complex token-concatenation happens correctly. Can get
        ...
        if not self.args.use_schema_lstm:

            column_out_padded = self.column_encoder(
                torch.cat(self.get_schema_span(column_hidden_states[0]), 0).unsqueeze(0))
            table_out_padded = self.table_encoder(
                torch.cat(self.get_schema_span(table_hidden_states[0]), 0).unsqueeze(0))
            value_out_padded = self.value_encoder(
                torch.cat(self.get_schema_span(value_hidden_states[0]), 0).unsqueeze(0))
        # pooling_output = self.pooled_encoder(pooling_output)
        ...
        # With "pad_sequence" we pay attention to the fact that each column can have a different amount of tokens (e.g. a 3-word column vs. a 1 word column), so we have to pad the shorter inputs.
            ...
            _, column_last_states, _ = self._rnn_wrapper(self.column_encoder, column_hidden_states_padded,
                                                         column_lengths)
            ...
            if list(flatten(value_hidden_states)):
                ...
                _, value_last_states, _ = self._rnn_wrapper(self.value_encoder, value_hidden_states_padded,
                                                            value_lengths)
        ...
        if not self.args.use_lstm_encoder:
            ...
            question_out, pooling_output = self.dropout(question_out[0]), (question_out[1][0], question_out[1][1])

        return question_out, column_out_padded, table_out_padded, value_out_padded, pooling_output

    def get_schema_span(self, hidden_states):
        averaged_span_hidden_states = []
        for i in hidden_states:
            i_span = torch.mean(i, keepdim=True, dim=0)
            averaged_span_hidden_states.append(i_span)
        return averaged_span_hidden_states

    def lstm_encode(self, src_token_embed, src_sents_len, q_onehot_project=None):
        """
    ...
    def _get_schema_hidden_states(last_hidden_states, all_schema_token_lengths, initial_pointers):
        ...
        for batch_itr_idx, (schema_token_lengths, initial_pointer) in enumerate(
                zip(all_schema_token_lengths, initial_pointers)):
            ...
            for schema_token_length in schema_token_lengths:
                ...
                hidden_states_schema.append(
                    last_hidden_states[batch_itr_idx, pointer: pointer + schema_token_length - 1, :])
    ...
    @staticmethod
    def _assert_all_elements_processed(all_question_span_lengths, all_column_token_lengths, all_table_token_lengths,
                                       all_value_token_lengths, last_pointers, len_last_hidden_states):
        ...
        for question_span_lengths, column_token_lengths, table_token_lengths, value_token_length, last_pointer in zip(
                all_question_span_lengths, all_column_token_lengths, all_table_token_lengths, all_value_token_lengths,
                last_pointers):
            assert sum(question_span_lengths) + sum(column_token_lengths) + sum(table_token_lengths) + sum(
                value_token_length) == last_pointer
