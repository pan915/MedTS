...
# from spider_utils.run_spider_dataset_utils import convert_id_to_db_name
# from model.modeling_sparc_lstm import TreeWithPara
...
class FromSQLParser(object):
    ...
    def get_val_id(self, where_val):
        ...
        for utter in self.interaction[:self.turn_level+1]:
            utterance_args.extend(utter['origin_utterance_arg'])
...
class FromAstToSql(object):
    ...
    # limit, union 解决了就可以了。
    # def expand_limit(limit:limit, rule_seq, rule_stack, rules, table_names, table_ids, column_name
    ...
    def update_table_name(self, expr, tab_dict):
        ...
        #         if k.upper() in flattened_str:
        #             flattened_str = flattened_str.replace(k.upper(), v)
        # elif len(tab_dict.keys()) == 1:
        #     for k, v in tab_dict.items():
        #         if k.upper() in flattened_str:
        #             flattened_str = flattened_str.replace(k.upper()+'.', '')
        if len(tab_dict.keys()) > 1:
            for k, v in tab_dict.items():
    ...
    def parse_select(self, ast, col_ids, tab_ids):
        ...
        for i in range(count):
            ...
            a_ast = l[i]
            # print(a_ast)
            a_expr = self.parse_a(a_ast, col_ids, tab_ids)
    ...
    def parse_a(self, ast, col_ids, tab_ids):
        a_expr = []
        rule_id = ast.rule_id
        rule = self.rule_dict[rule_id]
        agg = str(rule.rhs()[0])
    ...
    def val_to_str(self, y_val):
        ...
        #     return 'val'
        # else:
        #     val_str = self.utter[eval(y_val)[0]: eval(y_val)[1]]
        #     if len(val_str) == 0:
        #         return 'val'
        #     return val_str
        return '\'val\''
    ...
    # def parse_from(self, col_ids):
    #     from src.run_sparc_dataset_utils import convert_id_to_db_name
    #
    #     from_expr = []
    #     db_name = convert_id_to_db_name(self.db_id)
    #     # schema = self.database_until.tables[db_name]
    #     # col_set = schema['column_names']
    #     # table_names = schema['table_names']table_names
    ...
    #         col_ids.pop(idx)
    #     if len(col_ids) > 0:
    #         path = self.database_until.get_joined_table_path(db_id=self.db_id, v_set=col_ids)
    #         if path is None:
    #             t_id = self.col_table_ids[col_ids[0]]
    #             t_name = table_names[t_id]
    #             from_expr.extend(['FROM', t_name])
    #         elif len(path) == 1:
    #             c_name = column_names[path[0]]
    #             t_id = self.col_table_ids[path[0]]
    #             t_name = table_names[t_id]
    #             from_expr.extend(['FROM',t_name])
    #         elif len(path) == 2:
    #                 c_name_1 = column_names[path[0]]
    #                 t_id_1 = self.col_table_ids[path[0]]
    #                 t_name_1 = table_names[t_id_1]
    #                 c_name_2 = column_names[path[1]]
    #                 t_id_2 = self.col_table_ids[path[1]]
    #                 t_name_2 = table_names[t_id_2]
    #                 from_expr.extend(['FROM', t_name_1, 'AS', 'T1', 'JOIN', t_name_2, 'AS', 'T2',
    #                                   'ON', 'T1.', c_name_1, '=', 'T2.', c_name_2 ])
    #         else:
    #
    #             for idx, c in enumerate(path):
    #                 if idx == len(path) - 1:
    #                     break
    #                 c_name_1 = column_names[path[idx]]
    #                 t_id_1 = self.col_table_ids[path[idx]]
    #                 t_name_1 = table_names[t_id_1]
    #                 c_name_2 = column_names[path[idx+1]]
    #                 t_id_2 = self.col_table_ids[path[idx+1]]
    #                 t_name_2 = table_names[t_id_2]
    #                 if idx == 0:
    #                     from_expr.extend(['FROM', t_name_1, 'AS', 'T', idx+1, 'JOIN', t_name_2, 'AS', 'T', idx+2,
    #                                       'ON', 'T', idx+1, '.', c_name_1, '=', 'T', idx+2, '.', c_name_2])
    #
    #                 else:
    #                     from_expr.extend(['JOIN', t_name_2, 'AS', 'T', idx + 2,
    #                                       'ON', 'T', idx + 1, '.', c_name_1, '=', 'T', idx + 2, '.', c_name_2])
    #     else:
    #         pass
    #     return from_expr
    ...
    if len(join_clause) == 1:
        ...
        if (ft, tt) not in relations.keys():
            if ft > len(primary_keys) - 1 or tt > len(primary_keys) - 1:
                ...
                # join_con = 'ON %s.%s = %s.%s' % (join_clause[0][1].upper(), schema['column_names_original'][primary_keys[ft]][1],
                #                              join_clause[1][1].upper(), schema['column_names_original'][primary_keys[tt]][1])
                join_con = 'ON %s.%s = %s.%s' % (
            ...
            # join_con = 'ON %s.%s = %s.%s' % (join_clause[0][1].upper(), schema['column_names_original'][link[0]][1],
            #                                  join_clause[1][1].upper(), schema['column_names_original'][link[1]][1])
        ...
        # from_str = ' JOIN '.join(['%s AS %s' % (jc[0].upper(), jc[1].upper()) for jc in join_clause])
        from_str = ' JOIN '.join(['%s AS %s' % (jc[0], jc[1]) for jc in join_clause])
        ...
        for i in range(len(join_clause)):
            if i == 0:
                # from_str_list.append('%s AS %s' % (join_clause[i][0].upper(), join_clause[i][1]))
                ...
                if (ft, tt) not in relations.keys():
                    if ft > len(primary_keys) - 1 or tt > len(primary_keys) - 1 :
                        ...
                        # from_str_list.append('%s AS %s ON %s.%s = %s.%s'%(join_clause[i][0].upper(),
                        #                                                     join_clause[i][1].upper(),
                        #                                                     join_clause[i - 1][1].upper(),
                        #                                                     schema['column_names_original'][primary_keys[ft]][1],
                        #                                                     join_clause[i][1].upper(),
                        #                                                     schema['column_names_original'][primary_keys[tt]][1]))
        ...
        from_str = ' JOIN '.join(from_str_list)
