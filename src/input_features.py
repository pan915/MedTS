...
def encode_utter(question_spans, tokenizer, max_length_model, device):
    all_input_ids = []
    all_attention_mask = []
    all_segment_ids = []
    ...
    return input_ids_tensor, attention_mask_tensor, segment_ids_tensor, all_question_span_lengths
...
def _tokenize_column_names(column_name, tokenizer, do_sub_tokenizing=True):
    column_token_lengths = []
    all_column_tokens = []
    column_names = deepcopy(column_name)
